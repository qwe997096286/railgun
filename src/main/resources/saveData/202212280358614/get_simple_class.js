...
urlConfig = {
    baseUrl : urlroot + "/api/$po.lowCamelPOName"

}
var headerAuth = {Authorization: "Bearer eyJhbGciOiJIUzUxMiJ9.eyJqdGkiOiI1ZjY4NmE3YWEwYjc0ZjE3ODRlZTIyNDAzMjBhZjI3NyIsInVzZXIiOiJhZG1pbiIsInN1YiI6ImFkbWluIn0.NQQz7IHMOFqWoTgUltXxSSrRPvh_OE3sY6im0iz2kOcKi7vBo7oDz8FfvCB2rX4yz-8EpgH-MNlVour88EDxvg"}
...
#foreach($element in $dto.fields)
#if($element.hasLabel("SHOW") && $element.hasLabel("PAGING"))
      ,{field:'$element.name', title:'$element.comment', sort: true}
#end
#end
#if($!config.hasEdit || $!config.hasDel)
  ...
  renderObject2Form = function (obj) {
    var input = null;
#foreach($element in $dto.fields)
#if($element.hasLabel("SHOW"))
    $('#saveForm [name="$element.name"]').val(obj.$element.name);
#end
#end
  }
  #end
...
#set($else1="")
    ...
    if(obj.event === 'del'){
      ...
      #set($else1="else ")
    }#end#if($!config.hasEdit)
    $!{else1}if(obj.event === 'edit'){
#foreach($element in $dto.fields)
#if(!$element.hasLabel("EDIT") && $element.hasLabel("SHOW"))
      $('#saveForm [name="$!element.name"]').attr('disabled', true).addClass("btn-disabled");
#end
#end
      $('#saveForm').removeClass("layui-hide")
      renderObject2Form(data);
      layer.open({
        title:'保存',
        type: 1,
        ...
        id: 'save', //定义 ID，防止重复弹出
        btn:["确定", "取消"],
        btnAlign:'c',
        yes: function(index, layero) {
          $('#saveForm').addClass("layui-hide");
...
#if($!config.hasEdit || $!config.hasCreate)
  getFormData = function () {
    var input = null;
    var obj = {};
#foreach($element in $dto.fields)
#if($element.hasLabel("SHOW"))
    obj.$element.name = $('#saveForm [name="$element.name"]').val();
#end
#end
  return obj;
  }
